export default function Head() {
  return (
    <>
      <title>HASTI</title>
      <meta name="description" content="Home Assistant Store, Themes & Integrations" />
      <link rel="icon" href="/black_ha_cube_hasti_splled_out_op4_icon.png" />
      <meta name="theme-color" id="theme-color" content="#26282B"/>
      <meta name="viewport" content="user-scalable=no, initial-scale=1, minimum-scale=1, maximum-scale=1, width=device-width, target-densitydpi=device-dpi"/>
      <meta name="apple-mobile-web-app-status-bar-style" content="#26282B"/>

    </>
  )
}
